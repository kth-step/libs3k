...
s3k_cap_t s3k_mk_memory(s3k_addr_t bgn, s3k_addr_t end, s3k_rwx_t rwx) {
  ...
  s3k_block_t bgn_block =
      (bgn - (tag << S3K_MAX_BLOCK_SIZE)) >> S3K_MIN_BLOCK_SIZE;
  s3k_block_t end_block =
      (end - (tag << S3K_MAX_BLOCK_SIZE)) >> S3K_MIN_BLOCK_SIZE;

  return (s3k_cap_t){.mem = {
...
static bool s3k_cap_chan_derivable(s3k_cap_t p, s3k_cap_t c) {
  if (c.type == S3K_CAPTY_SOCKET) {
    return (c.sock.tag == 0) && is_range_subset(p.chan.mrk, p.chan.end,
                                                c.sock.chan, c.sock.chan + 1);
  }
  return (c.type == S3K_CAPTY_CHANNEL) &&
